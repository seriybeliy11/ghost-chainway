...
export default function EventCard({ event, index, onClick }: EventCardProps) {
  ...
  const getCategoryStyle = (category: string) => {
    ...
    return darkStyles[category] || darkStyles['Trending'];
  };

  const bgGrad = darkBgGradients[index % darkBgGradients.length];
  ...
  const noColor = 'from-teal-400 to-cyan-500';
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`relative rounded-2xl overflow-hidden cursor-pointer active:scale-[0.98] transition-shadow duration-300 h-[190px] border border-white/[0.08] hover:border-white/[0.15] hover:shadow-[0_8px_32px_rgba(57,174,207,0.1)]`}
    >
      ...
      <div className={`absolute inset-0 bg-gradient-to-br ${bgGrad}`}>
        ...
        {/* Glass overlay */}
        <div className={`absolute inset-0 transition-colors duration-300 bg-gradient-to-b from-black/15 via-black/35 to-black/80`} />
      </div>
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-4">
        ...
        <div className="flex-1 flex items-center">
          <h3 className={`text-[15px] font-bold leading-snug line-clamp-2 drop-shadow-lg transition-colors duration-300 text-white/95`}>
            {event.question}
          </h3>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="flex items-end justify-between mb-1">
            <div>
              ...
              <span className={`text-[10px] font-semibold ml-1.5 transition-colors duration-300 text-white/40`}>
                {event.outcomes?.[0] || 'Yes'}
              </span>
            </div>
            <div className="text-right">
              <span className={`text-[22px] font-extrabold leading-none transition-colors duration-300 text-teal-400`}>
                {noPrice.toFixed(0)}<span className="text-[13px] font-bold">%</span>
              </span>
              <span className={`text-[10px] font-semibold mr-1.5 transition-colors duration-300 text-white/40`}>
                {event.outcomes?.[1] || 'No'}
              </span>
            </div>
          </div>
          ...
          {/* Probability bar */}
          <div className={`h-2 rounded-full overflow-hidden flex transition-colors duration-300 bg-white/8`}>
            <motion.div
              initial={{ width: 0 }}
          ...
          {/* Footer stats */}
          <div className="flex items-center justify-between">
            <div className={`flex items-center gap-1 transition-colors duration-300 text-white/40`}>
